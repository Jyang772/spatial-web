...
var cell_ = []; //Holds the voronoi cell
	        

//Check if gridCell is inside polygon        
function inside(point,vs) {
    ...
    for(var i=1; i<vs.length; i++) {
        var xi = vs[i-1].x, yi = vs[i-1].y;
        var xj = vs[i].x, yj = vs[i].y;
    
        if(yi == canvasWidth || yj == canvasWidth){
            var intersect = ((yi >= y) != (yj >= y))
                && (x < (xj - xi) * (y - yi) / (yj - yi) + xi);
            if (intersect) inside_ = !inside_;
        }
        else { 
            var intersect = ((yi > y) != (yj > y))
                && (x < (xj - xi) * (y - yi) / (yj - yi) + xi);
...
//Check if polygon is inside gridCell
function inside2(point, vs) {
    var x = point[0], y = point[1];
    var inside_ = false;
    
    for(var i=1; i<vs.length; i++) {
        var xi = vs[i-1][0], yi = vs[i-1][1];
        var xj = vs[i][0], yj = vs[i][1];
...
var PIR = function(level,e,offsetLeft,offsetTop) {
	    ...
	    Voronoi.init();
	    drawBoard(level);
	    console.log(canvas2grid_(e.pageX-offsetLeft,e.pageY-offsetTop,level));
	    Voronoi.drawCells();
	    ...
	    for(var i=0; i<Math.pow(2,level); i++) {
	        for(var j=0; j<Math.pow(2,level); j++) {
	            ...
	            var point = [];
	            var x0 = j*(canvasWidth)/Math.pow(2,level);
	            var y0 = i*(canvasHeight)/Math.pow(2,level);
	            point.push(x0);
	            point.push(y0);
	            g.corners.push(point);
	            var x1 = j*(canvasWidth)/Math.pow(2,level);
	            var y1 = i*(canvasHeight)/Math.pow(2,level) + (canvasHeight)/Math.pow(2,level);
	            point = [];
	            point.push(x1);
	            point.push(y1);
	            g.corners.push(point);
	            var x2 = j*(canvasWidth)/Math.pow(2,level) + (canvasWidth)/Math.pow(2,level);
	            var y2 = i*(canvasHeight)/Math.pow(2,level) + (canvasHeight)/Math.pow(2,level);
	            point = [];
	            point.push(x2);
	            point.push(y2);
	            g.corners.push(point);
	            var x3 = j*(canvasWidth)/Math.pow(2,level) + (canvasWidth)/Math.pow(2,level);
	            var y3 = i*(canvasHeight)/Math.pow(2,level);
	            point = [];
	            point.push(x3);
	            point.push(y3);
	            g.corners.push(point);
	   ...
	   /*for(var i=0; i<arr.length; i++) {
	    ...
	    drawRect({
		x:gridCells[cell].corners[0][0],
		y:gridCells[cell].corners[0][1],
		width:canvasWidth/Math.pow(2,level),
		height:canvasWidth/Math.pow(2,level)
	    })
	    highlight(arr[cell].voronoi);
	    clearText();
	    appendNewText(arr[cell].voronoi);
...
var Voronoi = {
	...
	PI: self.Math.PI,
	isNaN: self.isNaN,
	DEFAULT_CANVAS_WIDTH: canvasWidth,
	DEFAULT_CANVAS_HEIGHT: canvasHeight,
	canvas: null,
	canvasMargin: 0,
	bbox: {xl:0,xr:canvasWidth,yt:0,yb:canvasHeight},
	...
	findInsertionPoint: function(x, sweep) {
		...
		while (l<r) {
			...
			// check if x after right break pointa
	...
	initCanvas: function() {
	    console.log("Voronoi - initCanvas")
		...
		var me = this;
	        for(var i in circles) {
	            me.addSite(circles[i].x,circles[i].y);
	        }
	...
	drawSites: function(ctx) {
		...
		for(var i in circles)
		    draw(context, circles[i].x, circles[i].y, "black", circles[i].radius, 0, "#003300", "black", "center", "bold 32px Arial",i,true);
	...
	*/
	...
	drawCells: function() {
		...
		for (var cellid in cells) {
			...
			//console.log("Circle id: " + (cellid-1));// + "Circle id: " + cellid-1);
			...
			for (iHalfedge=0; iHalfedge<nHalfedges; iHalfedge++) {
				...
				//if(iHalfedge < nHalfedges-1) //Don't print duplicate endpoint
				    c.cellEdges.push(v);
				}
		...
		for(var x in cell_) {
		    console.log("Cell id: " + x);
		    console.log("Circle id: " + x);
			    for(var j in cell_[x].cellEdges) {
				    console.log(cell_[x].cellEdges[j]);
			    }
			}
		},
	...
	drawEdges: function(ctx) {
	    ctx.save(); //For grid lines to remain gray
		ctx.beginPath();
		ctx.lineWidth=1; //0.5
		ctx.globalAlpha=1;
		var nEdges=this.edges.length;
		...
		for (var iEdge=0; iEdge<nEdges; iEdge++) {
			...
			va = edge.va;
			vb = edge.vb;
			//console.log("Start: " + va.x + "," + vb.y);
			//console.log("End: " + vb.x + "," + vb.y);
